...
namespace <xsl:value-of select="@unitTestNameSpace"/>.Base
{
  [TestClass]
  public class <xsl:value-of select="@name"/>BaseTest
    {
  ...
  <xsl:template name="resolveRecursiveParams">
    <xsl:apply-templates select="fields/field" mode="params"></xsl:apply-templates><xsl:if test="@type = 'dependent' or @type = 'abstractdependent'">
      ...
      <xsl:for-each select="/solution/entities/entity[@name=$varBaseEntity]"><xsl:if test="$varFieldCount > 0 and count(/solution/entities/entity[@name=$varBaseEntity]/fields/field) > 0">, </xsl:if><xsl:call-template name="resolveRecursiveParams"></xsl:call-template></xsl:for-each>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="resolveRecursiveParamsUpdate">    
    <xsl:apply-templates select="fields/field" mode="update"></xsl:apply-templates><xsl:if test="@type = 'dependent' or @type = 'abstractdependent'">
      <xsl:variable name="varBaseEntity" select="@baseEntity"></xsl:variable>
      <xsl:variable name="varFieldCount" select="count(fields/field)"></xsl:variable>
      <xsl:for-each select="/solution/entities/entity[@name=$varBaseEntity]"><xsl:call-template name="resolveRecursiveParamsUpdate"></xsl:call-template></xsl:for-each>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="resolveRecursiveParamsVerifyUpdate">    
    ...
    <xsl:if test="@type = 'dependent' or @type = 'abstractdependent'">      
      <xsl:variable name="varBaseEntity" select="@baseEntity"></xsl:variable>
      <xsl:variable name="varFieldCount" select="count(fields/field)"></xsl:variable>
      <xsl:for-each select="/solution/entities/entity[@name=$varBaseEntity]"><xsl:call-template name="resolveRecursiveParamsVerifyUpdate"></xsl:call-template></xsl:for-each>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="fields/field" mode="update">, <xsl:choose><xsl:when test="@edmType='string'"><xsl:variable name="Type" select="@type"/><xsl:choose><xsl:when test="count(//solution/userTypes/*[@name=$Type]/enumeration) > 0">"<xsl:value-of select="//solution/userTypes/*[@name=$Type]/enumeration[2]"/>"</xsl:when><xsl:otherwise>"<xsl:value-of select="@name"/>_2"</xsl:otherwise></xsl:choose></xsl:when><xsl:when test="@edmType = 'DateTime'">new DateTime(2010, 2, 2)</xsl:when><xsl:when test="@edmType = 'bool'">false</xsl:when><xsl:otherwise>2</xsl:otherwise></xsl:choose></xsl:template>

  <xsl:template match="fields/field" mode="verifyUpdate">
      Assert.AreEqual(target.<xsl:value-of select="@name"/>, <xsl:choose><xsl:when test="@edmType='string'"><xsl:variable name="Type" select="@type"/><xsl:choose><xsl:when test="count(//solution/userTypes/*[@name=$Type]/enumeration) > 0">"<xsl:value-of select="//solution/userTypes/*[@name=$Type]/enumeration[2]"/>"</xsl:when><xsl:otherwise>"<xsl:value-of select="@name"/>_2"</xsl:otherwise></xsl:choose></xsl:when><xsl:when test="@edmType = 'DateTime'">new DateTime(2010, 2, 2)</xsl:when><xsl:when test="@edmType = 'bool'">false</xsl:when><xsl:otherwise>2</xsl:otherwise></xsl:choose>);</xsl:template>
