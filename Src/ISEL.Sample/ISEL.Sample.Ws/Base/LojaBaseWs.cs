...
namespace ISEL.Sample.Ws.Base
{
    ...
    public class LojaBaseWs : System.Web.Services.WebService
    {
        [WebMethod]
        public long Create(string Nome)
        {        
            return Singleton<LojaService>.Current.Create(Nome);
        }

        [WebMethod]
        public void Update(long recordId, string Nome)
        {
            Singleton<LojaService>.Current.Update(recordId, Nome);
        }
